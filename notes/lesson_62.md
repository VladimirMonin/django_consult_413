...
Именно поэтому сигналы **не являются заменой** для систем очередей задач, таких как Celery или Redis Queue (RQ). Если вам нужно выполнить ресурсоемкую или длительную операцию в фоновом режиме, не блокируя пользователя, следует использовать очередь задач. Сигнал в этом случае может выступить в роли "триггера", который поставит задачу в очередь, но сама задача будет выполняться асинхронно в отдельном процессе.

В следующей части мы подробно рассмотрим, какие типы сигналов существуют в Django и как с ними работать.

## Типы сигналов 📊

Django предоставляет множество встроенных сигналов, которые можно разделить на несколько категорий в зависимости от того, какая часть фреймворка их отправляет. Ниже представлена сводная таблица с наиболее важными из них, отсортированная по частоте использования.

| Сигнал | Краткое описание | Основные аргументы и их описание | Пример обработчика |
| :--- | :--- | :--- | :--- |
| **Сигналы моделей (`django.db.models.signals`)** |
| `pre_save` | Отправляется перед сохранением модели. | `sender`: Класс модели.<br>`instance`: Экземпляр, который сохраняется.<br>`raw`: `True`, если данные загружаются из фикстур.<br>`using`: Псевдоним используемой БД.<br>`update_fields`: Набор полей для обновления. | `def h(sender, instance, **kwargs):` |
| `post_save` | Отправляется после сохранения модели. | `sender`: Класс модели.<br>`instance`: Сохраненный экземпляр.<br>`created`: `True`, если была создана новая запись.<br>`raw`: `True`, если данные из фикстур.<br>`using`: Псевдоним используемой БД.<br>`update_fields`: Набор обновленных полей. | `def h(sender, instance, created, **kwargs):` |
| `pre_delete` | Отправляется перед удалением модели. | `sender`: Класс модели.<br>`instance`: Удаляемый экземпляр.<br>`using`: Псевдоним используемой БД.<br>`origin`: Объект, инициировавший удаление. | `def h(sender, instance, **kwargs):` |
| `post_delete` | Отправляется после удаления модели. | `sender`: Класс модели.<br>`instance`: Удаленный экземпляр.<br>`using`: Псевдоним используемой БД.<br>`origin`: Объект, инициировавший удаление. | `def h(sender, instance, **kwargs):` |
| `m2m_changed` | Отправляется при изменении `ManyToManyField`. | `sender`: Промежуточная модель `m2m`-связи.<br>`instance`: Экземпляр, у которого изменилась связь.<br>`action`: Тип действия (`pre_add`, `post_add` и т.д.).<br>`reverse`: `True`, если связь меняется с "обратной" стороны.<br>`model`: Класс модели, объекты которой добавляются/удаляются.<br>`pk_set`: Набор `id` затронутых объектов.<br>`using`: Псевдоним используемой БД. | `def h(sender, instance, action, **kwargs):` |
| `pre_init` | Отправляется в начале `__init__()` модели. | `sender`: Класс модели.<br>`args`, `kwargs`: Позиционные и именованные аргументы, переданные в `__init__()`. | `def h(sender, *args, **kwargs):` |
| `post_init` | Отправляется в конце `__init__()` модели. | `sender`: Класс модели.<br>`instance`: Созданный экземпляр. | `def h(sender, instance, **kwargs):` |
| **Сигналы запроса/ответа (`django.core.signals`)** |
| `request_started` | Отправляется при начале обработки запроса. | `sender`: Класс обработчика запроса.<br>`environ`: Словарь `WSGI` окружения. | `def h(sender, environ, **kwargs):` |
| `request_finished` | Отправляется после отправки ответа клиенту. | `sender`: Класс обработчика запроса. | `def h(sender, **kwargs):` |
| `got_request_exception` | Отправляется при исключении в запросе. | `sender`: Не используется (`None`).<br>`request`: Объект `HttpRequest`. | `def h(sender, request, **kwargs):` |
| **Сигналы аутентификации (`django.contrib.auth.signals`)** |
| `user_logged_in` | Отправляется после успешного входа. | `sender`: Класс пользователя.<br>`request`: Объект `HttpRequest`.<br>`user`: Экземпляр вошедшего пользователя. | `def h(sender, request, user, **kwargs):` |
| `user_logged_out` | Отправляется после выхода пользователя. | `sender`: Класс пользователя.<br>`request`: Объект `HttpRequest`.<br>`user`: Экземпляр вышедшего пользователя. | `def h(sender, request, user, **kwargs):` |
| `user_login_failed` | Отправляется при неудачной попытке входа. | `sender`: Имя модуля аутентификации.<br>`credentials`: Учетные данные.<br>`request`: Объект `HttpRequest`. | `def h(sender, credentials, request, **kwargs):` |
| **Сигналы миграций (`django.db.models.signals`)** |
| `pre_migrate` | Отправляется перед выполнением миграций. | `sender`: `AppConfig` приложения.<br>`app_config`: То же, что и `sender`.<br>`plan`: План выполнения миграций. | `def h(sender, app_config, **kwargs):` |
| `post_migrate` | Отправляется после выполнения миграций. | `sender`: `AppConfig` приложения.<br>`app_config`: То же, что и `sender`.<br>`plan`: План выполнения миграций. | `def h(sender, app_config, **kwargs):` |
| **Прочие сигналы** |
| `connection_created` | Отправляется при создании соединения с БД. | `sender`: Класс-обертка для БД.<br>`connection`: Открытое соединение. | `def h(sender, connection, **kwargs):` |
| `setting_changed` | Отправляется при изменении настроек в тестах. | `sender`: `TestCase` или `None`.<br>`setting`: Имя настройки.<br>`value`: Новое значение.<br>`enter`: `True` при установке, `False` при сбросе. | `def h(sender, setting, value, **kwargs):` |
| `template_rendered` | Отправляется при рендеринге шаблона в тестах. | `sender`: Объект `Template`.<br>`template`: То же, что и `sender`.<br>`context`: Контекст рендеринга. | `def h(sender, template, context, **kwargs):` |
...
Как видите, модель содержит стандартные поля для отзыва: `text` для самого сообщения и `rating` для оценки. Однако самое интересное для нас поле — это `status`.
...
**Когда нужно переходить на очередь задач?**
...
Проблема кроется в **порядке выполнения операций** в Django. Когда вы сохраняете объект вместе с его M2M-связями (например, через `ModelForm` или админку), происходит следующее:
