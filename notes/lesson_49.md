...
(описать стуркуру проекта ASCII текстовой схемой!!!)

Важно то что Django обожает пакеты, и в большинстве случаев `__init__.py` нужно делать и следить чтобы они были в подпапках.
...
Создаем 2 файла в корне проекта.

Первый файл `.env` для хранения переменных окружения. Его обязательно сразу добавим в `.gitignore`
Второй, его зеркало, но БЕЗ секретных данных, `.env.example` для примера.

В них переменные будут одинаковые, но второй файл просто для примера - его пушат на гитхаб.

В файле `.env` добавим переменную `SECRET_KEY` и `MISTRAL_API_KEY`
Впишем реальные ключи.

Незабудем и про файл примера:
SECRET_KEY=django_secret_key
MISTRAL_API_KEY=mistral_api_key

Обязательно добавим в `.gitignore` файл `.env`
