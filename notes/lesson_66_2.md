...
### Параметр `Next` в `LoginView`

#TODO Расписать эту часть детально. Не менее 5 заголовков 4 уровня в каждом не менее 10 предложений. В каждом заголовке схемка или диаграммка.

/services/ - защищен от неавторизованных пользователей
/users/login/?next=/services/ - вас кинет по маршруту LOGIN_URL но не просто, а еще вложат в GET параметр next=/services/
После успешной авторизации у вас будет автоматический редирект на маршрут из параметра next и вы посмотрите на то, что хотели изначально увидеть
...
Если мы пишем функциональную вью выхода, мы можем сделать так, что она будетобрабатывать GET или POST на наш выбор. 
Однако если мы используем  LogoutView, то она будет обрабатывать только POST-запросы.

Отправить POST запрос и разлогинить пользователя через браузер у нас есть только один способ это сделать.

Форма! Форма с кнопкой отправки POST запроса + CSRF токен.
При этом эта форма может выглядить как кнопка или даже ссылка!
